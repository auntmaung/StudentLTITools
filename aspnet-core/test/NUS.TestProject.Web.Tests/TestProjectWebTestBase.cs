...
using NUS.TestProject.Models.TokenAuth;
using NUS.TestProject.Web.Startup;
...
namespace NUS.TestProject.Web.Tests
{
    public abstract class TestProjectWebTestBase : AbpAspNetCoreIntegratedTestBase<Startup>
    {
        protected static readonly Lazy<string> ContentRootFolder;

        static TestProjectWebTestBase()
        {
        ...
        protected override IWebHostBuilder CreateWebHostBuilder()
        {
            return base
                ...
                .UseSetting(WebHostDefaults.ApplicationKey, typeof(TestProjectWebMvcModule).Assembly.FullName);
        }
        ...
        {
            using (var context = IocManager.Resolve<TestProjectDbContext>())
            {
                action(context);
                context.SaveChanges();
            }
        ...
        protected async Task UsingDbContextAsync(Func<TestProjectDbContext, Task> action)
        {
            using (var context = IocManager.Resolve<TestProjectDbContext>())
            {
        ...
        protected async Task<T> UsingDbContextAsync<T>(Func<TestProjectDbContext, Task<T>> func)
        {
            T result;
